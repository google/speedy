...
ComputeSpectrogram(std::vector<int16>input_vector, float sample_rate) {
...
void SaveWaveform(std::vector<int16> samples, const std::string& filename,
...
std::vector<int16> ReadWaveFile(const std::string& fileName, int* sampleRate,
                                int* numChannels) {
...
std::vector<int16> CreateSinusoidTest(int sampleRate, int channels,
                                      int matchingChannels,
                                      float num_seconds) {
  constexpr uint32 kAmplitude = 32000;
  uint32 kTotalSampleCount = num_seconds*sampleRate;
  float kPeriodSamples = static_cast<float>(sampleRate) / kPitch;
  std::vector<int16> inputVector;

  // Create the input sinsuoid
  for (uint32 i=0; i < kTotalSampleCount; ++i) {
    int16 sample = static_cast<int16>(kAmplitude * sin(i * 2 * M_PI /
                                                       kPeriodSamples));
    inputVector.push_back(sample);    // First channel is always loaded.
    for (uint32 j=1; j < channels; j++) {
...
std::vector<float> CreateSinusoidFloatTest(int sampleRate, int channels,
  ...
  uint32 kTotalSampleCount = sampleRate;
...
std::vector<int16>TimeCompressVector(sonicStream stream,
                                     std::vector<int16>inputVector,
                                     float speed,
                                     float nonlinear) {
  EXPECT_TRUE(stream);
  constexpr uint64 kBufferSize = 128;
  int32 samplesRead;
  int32 channelCount = sonicIntGetNumChannels(stream);
  int16* outputBuffer = new int16[kBufferSize*channelCount];
  std::vector<int16> outputVector;

  sonicSetSpeed(stream, speed);
  // TODO(malcolmslaney) set last parameter to 0.0
  sonicEnableNonlinearSpeedup(stream, nonlinear, 0.0);
  ...
  for (uint32 t = 0; t < num_time_steps; t += kBufferSize) {
    int16 *inputPointer = &inputVector[0] + channelCount*t;
    int64 inputCount = fmin(kBufferSize, num_time_steps-t);
    ...
    for (uint32 i=0; i < samplesRead*channelCount; ++i) {
...
std::vector<float>TimeCompressFloatVector(sonicStream stream,
                                          ...
                                          float nonlinear) {
  EXPECT_TRUE(stream);
  constexpr uint64 kBufferSize = 128;  // Number of time steps
  int32 samplesRead;
  int32 channelCount = sonicIntGetNumChannels(stream);
...
TEST_F(Sonic2Test, TestWithSinusoids) {
  ...
  WriteData(teagerVector, "/tmp/sounds/sinusoid-input-teager.txt");
  ...
  SaveWaveform(compressed_result, "/tmp/sounds/sinusoid-output.wav",
  ...
  WriteData(teagerVector, "/tmp/sounds/sinusoid-output-teager.txt");
...
TEST_F(Sonic2Test, TestStereoOriginalSonic) {
  ...
  sonicStream stream = sonicIntCreateStream(kSampleRate, kNumChannels);
  sonicIntSetSampleRate(stream, kSampleRate);
  sonicIntSetSpeed(stream, kSpeed);

  auto testStereo = CreateSinusoidTest(kSampleRate, kNumChannels, 1, 1.0);
  EXPECT_TRUE(sonicIntWriteShortToStream(stream, &testStereo[0],
                                         testStereo.size()/kNumChannels));

  constexpr int kBufferSize = 1024;
  int16 buffer[kBufferSize*kNumChannels];
  std::vector<int16> compressed;
  int samplesRead = 0;
  do {
    samplesRead = sonicIntReadShortFromStream(stream, buffer, kBufferSize);
    for (int i = 0; i < samplesRead*kNumChannels; i++) {
      compressed.push_back(buffer[i]);
    }
    sonicIntFlushStream(stream);
  } while (samplesRead > 0);
...
TEST_F(Sonic2Test, TestStereoSinusoid) {
  ...
  // Now check to see that the right channel is coherent.
  std::vector<int16> rightDichotic;
...
TEST_F(Sonic2Test, TestStereoTapestry) {
  ...
  for (uint32 i=0; i < original_samples.size(); ++i) {
    int16 sample = original_samples[i];
  ...
  for (uint32 t=0; t < stereo_result.size()/channelCount; ++t) {
...
TEST_P(Sonic2ParameterizedTest, TestWithVaryingSpeed) {
  ...
  int32 samplesRead;
  int16* outputBuffer = new int16[kBufferSize*kNumChannels];
  std::vector<int16> outputVector;

  Initialize(kSampleRate, kNumChannels);
  sonicEnableNonlinearSpeedup(stream_, 0, 0.0);
  ...
  for (uint32 t = 0; t < num_time_steps; t += kBufferSize) {
    int16 *inputPointer = &sinusoid[0] + kNumChannels*t;
    int64 inputCount = fmin(kBufferSize, num_time_steps-t);
    float speed;
    if (frame_count++ % 2){
      speed = speed1;
    ...
    for (uint32 i=0; i < samplesRead*kNumChannels; ++i) {
      outputVector.push_back(outputBuffer[i]);
    }
  }
  // Flush the processing streams and collect the rest of the samples.
